...
def genRoundKeys(key):
    ...
    for i in range(1, FULLROUND + 2):
        # use 64 leftmost bits
        keys.append(current_register >> 16)

        # rorate registers by 61 positions to left
        current_register = rol(current_register, 61, 80)

        # take first 4 bits and use sbox
        four_bits = rol(current_register, 4, 80) & 0xF
        four_bits_sbox = sbox[four_bits] << 4 * 19
        current_register = (current_register ^ (four_bits << 4 * 19)) | four_bits_sbox

        # round counter on 19th - 15th bits
        current_register = current_register ^ (i << 15)
...
def sBoxLayer(state):
    ...
    for i in range(16):
        hex = state & 0xF
        res = sbox[hex] << 4 * i | res
        state = state >> 4

    return res


def sBoxLayer_inverted(state):
    res = 0

    for i in range(16):
        hex = state & 0xF
        res = sbox.index(hex) << 4 * i | res
        state = state >> 4
...
def pLayer(state):
    ...
    for i in range(64):
        bit_value = state & 0b1
        final_placement = bit_value << pmt[i]
        res = final_placement | res
        state = state >> 1

    return res


def pLayer_inverted(state):
    res = 0

    for i in range(64):
        bit_value = state & 0b1
        final_placement = bit_value << pmt.index(i)
        res = final_placement | res
        state = state >> 1
...
def present_inv_round(state, roundKey):
    state = pLayer_inverted(state)
    state = sBoxLayer_inverted(state)
    state = addRoundKey(state, roundKey)
    return state
